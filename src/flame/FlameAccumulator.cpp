...
FlameAccumulator::FlameAccumulator(dim2 dims, const Rectangle &frame) : frame(frame), dims(dims) {
...
const Color
FlameAccumulator::color(const Palette &palette, const Color &background, unsigned int x, unsigned int y) const {
    if (x < 0 || x >= dims.width) {
        throw std::out_of_range("FlameAccumulator::intensity : x out of range");
    }
...
void FlameAccumulator::compute_max_hit_count() {
    max_hit_count = 0;
    for (unsigned int i = 0; i < dims.width; i++) {
        for (unsigned int j = 0; j < dims.height; j++) {
